...
{
    public class CommitmentsRelationshipRepository : BaseRepository, ICommitmentsRelationshipRepository
    {
        public CommitmentsRelationshipRepository(string connectionString) : base(connectionString)
        {
        }

        public async Task CreateCommitmentsRelationship(Relationship relationship)
        {
            await WithConnection(async c =>
            {
                ...
                return await c.ExecuteAsync(sql: "[Data_Load].[SaveCommitmentsRelationship]", param: parameters,
                    commandType: CommandType.StoredProcedure);
            });
        }

        public async Task VerifyCommitmentsRelationship(long providerId, long employerAccountId, string legalEntityId, bool? verified)
        {
            await WithConnection(async c =>
            {
                var parameters = new DynamicParameters();
